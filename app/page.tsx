...
export default function Landing() {
  ...
  // ADDED: missing state used by banner + modals
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-slate-100 text-slate-900 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/60 dark:bg-slate-900/70 border-b shadow-sm">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            <a
              href="#how"
              className="hover:text-slate-900/80 text-slate-600 dark:text-slate-300 transition-colors"
            >
              How it works
            </a>
            <a
      ...
      {/* Pilot banner for non-resort hosts */}
      <div className="bg-amber-50 dark:bg-amber-900/30 border-y border-amber-200 dark:border-amber-800">
        <div className="mx-auto max-w-7xl px-4 py-3 text-sm text-amber-800 dark:text-amber-200 flex flex-col sm:flex-row items-start sm:items-center gap-2 justify-between">
          <span>
            <strong>Phase 0 Pilot:</strong> You can list your personal home now (outside a resort)
            with safety + parking guardrails. Resorts come later.
          </span>
          {!user && (
          ...
          {user && (
            <Button size="sm" variant="secondary" onClick={() => setShowHostWizard(true)}>
              List your home
            </Button>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <motion.div variants={fade} initial="hidden" animate="show" className="space-y-6">
            <h1 className="text-3xl md:text-5xl font-bold tracking-tight">
              Restore community —{" "}
              <span className="text-indigo-600 dark:text-indigo-400">without schooling</span>
            </h1>
            <p className="text-slate-600 dark:text-slate-300 text-lg leading-relaxed">
              Book homeschool-friendly homes inside gated resort neighborhoods. Parent-led
              gatherings. No drop-offs. Zoning-safe by design.
            </p>
            ...
            <div className="flex flex-wrap gap-2 pt-2 text-sm">
              <span className="inline-flex items-center gap-2 rounded-full bg-white/70 dark:bg-slate-800/70 border px-3 py-1">
                <Shield className="h-4 w-4" /> Two adults per room
              </span>
              <span className="inline-flex items-center gap-2 rounded-full bg-white/70 dark:bg-slate-800/70 border px-3 py-1">
                <Lock className="h-4 w-4" /> Invite-based access
              </span>
              <span className="inline-flex items-center gap-2 rounded-full bg-white/70 dark:bg-slate-800/70 border px-3 py-1">
                <MapPin className="h-4 w-4" /> Zone-aware listings
              </span>
            </div>
          </motion.div>
            ...
            <div className="aspect-[4/3] rounded-2xl bg-gradient-to-br from-indigo-100 via-white to-emerald-100 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800 border shadow-inner grid place-items-center">
              <div className="text-center p-6">
                ...
                <div className="mt-2 text-xl font-semibold">
                  Gated resort layout • Central amenities • Overflow parking
                </div>
                <div className="mt-4 text-slate-600 dark:text-slate-300">
                  Auditorium • Gym • Baseball stadium • Teen courtyard • Airbnb homes cluster
                </div>
              </div>
            </div>
          </motion.div>
          ...
          {[
            {
              icon: Home,
              title: "Pick a home",
              text: "Choose a homeschool-ready home near the central amenities.",
            },
            {
              icon: Users2,
              title: "Add your group",
              text: "Parents stay. Two adults per room. No drop-offs.",
            },
            {
              icon: CalendarDays,
              title: "Book a time",
              text: "Morning, afternoon, or evening blocks — with simple repeats.",
            },
          ].map((item, i) => (
            <Card key={i}>
      ...
      <section id="safety" className="mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            {
              icon: Shield,
              title: "Two-adult coverage",
              text: "Every active room must have two adults. No exceptions.",
            },
            {
              icon: Lock,
              title: "Invite-based access",
              text: "Only known parents can book. Tutors join by parent invite.",
            },
            {
              icon: Check,
              title: "Zoning aware",
              text: "Parking + capacity logic keeps bookings compliant and calm.",
            },
          ].map((item, i) => (
            <Card key={i}>
              <CardHeader className="flex flex-row items-center gap-3">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <MapPin className="h-5 w-5" /> Central amenities
              </CardTitle>
            </CardHeader>
            <CardContent className="text-slate-600 dark:text-slate-300">
              Auditorium, gym, baseball stadium, and teen courtyard sit at the center — with
              overflow parking so residential streets stay quiet.
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Building2 className="h-5 w-5" /> Airbnb homes cluster
              </CardTitle>
            </CardHeader>
            <CardContent className="text-slate-600 dark:text-slate-300">
              Purpose-built homes for daily enrichment use. No beds. No overnight stays. Just
              beautiful, bookable spaces.
            </CardContent>
          </Card>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-6">
              Offer a homeschool-friendly home inside resort neighborhoods. Parking, capacity, and
              safety are built into the listing flow.
            </p>
            <ul className="space-y-2 text-slate-700 dark:text-slate-200">
              <li className="flex items-center gap-2">
                <Sparkles className="h-4 w-4" /> Open-concept kitchens with pizza + ice cream add-ons
              </li>
              <li className="flex items-center gap-2">
                <Users2 className="h-4 w-4" /> Parent-led groups only — no drop-offs
              </li>
              <li className="flex items-center gap-2">
                <Shield className="h-4 w-4" /> Two adults per room required
              </li>
            </ul>
          </div>
          <Card>
      ...
      <section id="dev" className="mx-auto max-w-7xl px-4 py-12">
        <div className="grid md:grid-cols-2 gap-6 items-start">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-6">
              License the platform for homeschool-friendly neighborhoods. Booking engine, safety
              logic, and zoning overlays included.
            </p>
            <ul className="space-y-2 text-slate-700 dark:text-slate-200">
              <li className="flex items-center gap-2">
                <MapPin className="h-4 w-4" /> Regrid / GIS integration
              </li>
              <li className="flex items-center gap-2">
                <Lock className="h-4 w-4" /> Private, zone-gated access
              </li>
              <li className="flex items-center gap-2">
                <HeartHandshake className="h-4 w-4" /> HOA-friendly governance
              </li>
            </ul>
          </div>
          <Card>
            ...
            <CardContent>
              <form
                ...
                onSubmit={(e) => {
                  e.preventDefault();
                  alert("Thanks! We'll reach out.");
                }}
              >
                <Input placeholder="Your name" required />
                <Input placeholder="Work email" type="email" required />
                <Textarea
                  placeholder="Tell us about your project (city, lots, amenities)"
                  rows={4}
                />
                <Button type="submit" className="w-full">
                  Contact me
          ...
          <CardContent>
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="grid md:grid-cols-3 gap-3">
                ...
                />
                <select
                  className="rounded-md border bg-white dark:bg-slate-900 dark:border-slate-700 px-3 py-2"
                  value={role}
      ...
      <section id="faq" className="mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent className="text-slate-600 dark:text-slate-300">
              No. It’s a booking tool for parent-led gatherings inside resort neighborhoods. No
              drop-offs. No curriculum. No classes sold.
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
            ...
            <CardContent className="text-slate-600 dark:text-slate-300">
              Two adults per room, invite-only access, and parking/capacity logic. Tutors join by
              parent invite only.
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent className="text-slate-600 dark:text-slate-300">
              Resort neighborhoods are planned for hospitality + enrichment use from day one.
              Overflow parking keeps streets clear.
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
            ...
            <CardContent className="text-slate-600 dark:text-slate-300">
              Yes. Submit interest above and we’ll reach out as new resorts open.
            </CardContent>
          </Card>
        </div>
      </section>
      ...
      <footer className="border-t mt-8">
        <div className="mx-auto max-w-7xl px-4 py-10 text-sm text-slate-600 dark:text-slate-300 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-lg bg-slate-900 text-white grid place-items-center font-bold text-xs dark:bg-white dark:text-slate-900">
              HR
            </div>
...
function LoginModal({
  ...
  onClose: () => void;
  onSuccess: (u: { email: string; name: string }) => void;
}) {
  const [email, setEmail] = useState("");
...
function HostWizard({
  ...
  const [form, setForm] = useState<{
    address: string;
    rooms: { name: string; adults: number }[];
    parkingSpots: number;
    availability: string;
    notes: string;
  }>({
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                We’ll use this to estimate legal parking + guest capacity. Exact address stays
                private until booking.
              </p>
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, parkingSpots: Number((e.target as HTMLInputElement).value) })
                }
              />
            </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              {form.rooms.map((r, i) => (
                <div key={i} className="grid grid-cols-2 gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const rooms = [...form.rooms];
                      rooms[i].adults = Number((e.target as HTMLInputElement).value);
                      setForm({ ...form, rooms });
                    }}
                  />
                </div>
              ...
              <div className="flex gap-2">
                ...
                <p className="text-xs text-slate-500 self-center">
                  Minimum 2 adults per room. No exceptions in pilot.
                </p>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Shoes off, use side entrance, bring water bottles…"
              />
            </div>
        ...
        {step === 3 && (
          <div className="space-y-3 text-center py-8">
            ...
            <p className="text-slate-600 dark:text-slate-300">
              We’ll run zoning + parking checks and email {user?.email} when your pilot listing is
              visible to families near you.
            </p>
